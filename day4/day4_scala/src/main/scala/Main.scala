...
object Main extends App {
	...
	def splitByPassport(l: List[String], acc: String = ""): List[String] = l match {
		case "" :: tail =>  acc + " " :: splitByPassport(tail) //at least one space after a value
		case head :: tail => splitByPassport(tail, acc + " " + head) //at least one space after value
		case _ => List(acc+ " ") //at least one space after value
	}

	def isPassport(potentialPassport: String): Boolean = 
		// if the `:` is missing it could be a value, not a key
